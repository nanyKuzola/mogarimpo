...
function switchLayout() {
	...
	layout.addEventListener(
		...
		function() {
			...
			if ( ['0', 'not_found', 'server_error', 'offline', 'individual'].includes(layout.value) ) {
...
function getMagicTags(groupValue) {
	...
	for( let index in groupValue ){
		...
		if ( neveCustomLayouts.magicTags[root].hasOwnProperty( 'general' ) ){
			allMagicTags = allMagicTags.concat( neveCustomLayouts.magicTags[root].general );
		}
