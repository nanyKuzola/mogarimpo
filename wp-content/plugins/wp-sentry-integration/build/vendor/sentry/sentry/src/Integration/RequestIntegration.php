...
final class RequestIntegration implements \Sentry\Integration\IntegrationInterface
{
     ...
     */
    public function __construct(?\Sentry\Options $options = null, ?\Sentry\Integration\RequestFetcherInterface $requestFetcher = null)
    {
        if (null !== $options) {
            @\trigger_error(\sprintf('Passing the options as argument of the constructor of the "%s" class is deprecated since version 2.1 and will not work in 3.0.', self::class), \E_USER_DEPRECATED);
        }
     ...
     */
    public static function applyToEvent(self $self, \Sentry\Event $event, ?\WPSentry\ScopedVendor\Psr\Http\Message\ServerRequestInterface $request = null) : void
    {
        @\trigger_error(\sprintf('The "%s" method is deprecated since version 2.1 and will be removed in 3.0.', __METHOD__), \E_USER_DEPRECATED);
    ...
    private function processEvent(\Sentry\Event $event, \Sentry\Options $options, ?\WPSentry\ScopedVendor\Psr\Http\Message\ServerRequestInterface $request = null) : void
    {
        if (null === $request) {
    ...
    private function removePiiFromHeaders(array $headers) : array
    {
        ...
        return \array_filter($headers, static function (string $key) use($keysToRemove) : bool {
            return !\in_array(\strtolower($key), $keysToRemove, \true);
     ...
     * @return mixed
     */
    private function captureRequestBody(\Sentry\Options $options, \WPSentry\ScopedVendor\Psr\Http\Message\ServerRequestInterface $serverRequest)
    {
        ...
        $requestData = \array_merge($this->parseUploadedFiles($serverRequest->getUploadedFiles()), \is_array($requestData) ? $requestData : []);
        if (!empty($requestData)) {
            return $requestData;
        }
        if ('application/json' === $serverRequest->getHeaderLine('Content-Type')) {
            try {
                return \Sentry\Util\JSON::decode($requestBody->getContents());
