...
final class HttpTransport implements \Sentry\Transport\TransportInterface, \Sentry\Transport\ClosableTransportInterface
{
     ...
     */
    public function __construct(\Sentry\Options $options, \WPSentry\ScopedVendor\Http\Client\HttpAsyncClient $httpClient, \WPSentry\ScopedVendor\Http\Message\RequestFactory $requestFactory, bool $delaySendingUntilShutdown = \true, bool $triggerDeprecation = \true, ?\WPSentry\ScopedVendor\Psr\Log\LoggerInterface $logger = null)
    {
    ...
    public function send(\Sentry\Event $event) : ?string
    {
        ...
        $request = $this->requestFactory->createRequest('POST', $dsn->getStoreApiEndpointUrl(), ['Content-Type' => 'application/json'], \Sentry\Util\JSON::encode($event->toArray()));
    ...
    private function cleanupPendingRequests() : void
    {
        ...
        try {
            $eachPromise = new \WPSentry\ScopedVendor\GuzzleHttp\Promise\EachPromise($requestGenerator(), ['concurrency' => 30, 'rejected' => function (\Throwable $exception, int $requestIndex) : void {
                $this->logger->error(\sprintf('Failed to send the event to Sentry. Reason: "%s".', $exception->getMessage()), ['exception' => $exception, 'event' => $this->pendingRequests[$requestIndex][1]]);
            }]);
            $eachPromise->promise()->wait();
        } catch (\Throwable $exception) {
            $this->logger->error(\sprintf('Failed to send the event to Sentry. Reason: "%s".', $exception->getMessage()), ['exception' => $exception]);
        }
