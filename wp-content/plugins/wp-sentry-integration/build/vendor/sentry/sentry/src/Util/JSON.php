...
final class JSON
{
    ...
    {
        if ($maxDepth < 0) {
            throw new \Sentry\Exception\JsonException('Reached the maximum depth limit while sanitizing the data.');
        }
        if (\is_string($data)) {
    ...
    private static function convertStringToUtf8(string $value) : string
    {
        ...
        $encoding = \mb_detect_encoding($value, \mb_detect_order(), \true);
