    ...
    public static function setupIntegrations(array $integrations) : array
    {
        ...
        foreach ($integrations as $integration) {
            ...
            if (!$integration instanceof \Sentry\Integration\IntegrationInterface) {
                throw new \InvalidArgumentException(\sprintf('Expecting integration implementing %s interface, got %s', \Sentry\Integration\IntegrationInterface::class, $className));
            }
            if (!isset(self::$integrations[$className])) {
                self::$integrations[$className] = \true;
