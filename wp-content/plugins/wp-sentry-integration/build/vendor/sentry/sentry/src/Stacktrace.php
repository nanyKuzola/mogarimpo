...
class Stacktrace implements \JsonSerializable
{
    ...
    private const ANONYMOUS_CLASS_PREFIX = "class@anonymous\0";
    /**
     ...
     */
    protected static $importStatements = ['include', 'include_once', 'require', 'require_once'];
    /**
     * Constructor.
     *
     ...
     * @return static
     */
    public static function createFromBacktrace(\Sentry\Options $options, \Sentry\Serializer\SerializerInterface $serializer, \Sentry\Serializer\RepresentationSerializerInterface $representationSerializer, array $backtrace, string $file, int $line)
    {
        $stacktrace = new static($options, $serializer, $representationSerializer, \func_num_args() > 6 ? \func_get_arg(6) : \true);
     ...
     */
    public function getFrame(int $index) : \Sentry\Frame
    {
    ...
    public function addFrame(string $file, int $line, array $backtraceFrame) : void
    {
        ...
        if (isset($backtraceFrame['class']) && isset($backtraceFrame['function'])) {
            ...
            $functionName = \sprintf('%s::%s', \preg_replace('/0x[a-fA-F0-9]+$/', '', $backtraceFrame['class']), $backtraceFrame['function']);
        ...
        $frame = new \Sentry\Frame($functionName, $this->stripPrefixFromFilePath($file), $line, $file, $frameArguments, $this->isFrameInApp($file, $functionName));
    ...
    protected function getSourceCodeExcerpt(string $path, int $lineNumber, int $maxLinesToFetch) : array
    {
        ...
        $frame = ['pre_context' => [], 'context_line' => '', 'post_context' => []];
    ...
    public function getFrameArguments(array $frame) : array
    {
        ...
        if (\in_array($frame['function'], static::$importStatements, \true)) {
            if (empty($frame['args'])) {
                return [];
            }
            return ['param1' => $this->serializeArgument($frame['args'][0])];
        }
        try {
