...
final class EventFactory implements \Sentry\EventFactoryInterface
{
    ...
    public function createWithStacktrace(array $payload) : \Sentry\Event
    {
        if (!isset($payload['stacktrace']) || !$payload['stacktrace'] instanceof \Sentry\Stacktrace) {
            $payload['stacktrace'] = \Sentry\Stacktrace::createFromBacktrace($this->options, $this->serializer, $this->representationSerializer, \debug_backtrace(\DEBUG_BACKTRACE_IGNORE_ARGS), __FILE__, __LINE__ - 6, \func_num_args() > 1 ? \func_get_arg(1) : \true);
        }
    ...
    public function create(array $payload) : \Sentry\Event
    {
        ...
        if (isset($payload['exception']) && $payload['exception'] instanceof \Throwable) {
            $this->addThrowableToEvent($event, $payload['exception'], \func_num_args() > 1 ? \func_get_arg(1) : \true);
        }
     ...
     *                                                 the integration
     */
    private function addThrowableToEvent(\Sentry\Event $event, \Throwable $exception, bool $shouldReadSourceCodeExcerpts) : void
    {
        ...
        do {
            $exceptions[] = ['type' => \get_class($currentException), 'value' => $currentException->getMessage(), 'stacktrace' => \Sentry\Stacktrace::createFromBacktrace($this->options, $this->serializer, $this->representationSerializer, $currentException->getTrace(), $currentException->getFile(), $currentException->getLine(), $shouldReadSourceCodeExcerpts)];
