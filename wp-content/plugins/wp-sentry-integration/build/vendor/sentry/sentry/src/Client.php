 ...
 */
final class Client implements \Sentry\FlushableClientInterface
{
    /**
     ...
     */
    public function __construct(\Sentry\Options $options, \Sentry\Transport\TransportInterface $transport, \Sentry\EventFactoryInterface $eventFactory, ?\WPSentry\ScopedVendor\Psr\Log\LoggerInterface $logger = null)
    {
        $this->options = $options;
        $this->transport = $transport;
        $this->eventFactory = $eventFactory;
        $this->integrations = \Sentry\Integration\Handler::setupIntegrations($options->getIntegrations());
     ...
     * {@inheritdoc}
     */
    public function captureMessage(string $message, ?\Sentry\Severity $level = null, ?\Sentry\State\Scope $scope = null) : ?string
    {
     ...
     * {@inheritdoc}
     */
    public function captureException(\Throwable $exception, ?\Sentry\State\Scope $scope = null) : ?string
    {
        if (!isset($this->integrations[\Sentry\Integration\IgnoreErrorsIntegration::class]) && $this->options->isExcludedException($exception, \false)) {
            return null;
        }
     ...
     * {@inheritdoc}
     */
    public function captureEvent(array $payload, ?\Sentry\State\Scope $scope = null) : ?string
    {
     ...
     * {@inheritdoc}
     */
    public function captureLastError(?\Sentry\State\Scope $scope = null) : ?string
    {
    ...
    {
        if (!$this->transport instanceof \Sentry\Transport\ClosableTransportInterface) {
     ...
     */
    private function prepareEvent(array $payload, ?\Sentry\State\Scope $scope = null) : ?\Sentry\Event
    {
        $shouldReadSourceCodeExcerpts = !isset($this->integrations[\Sentry\Integration\FrameContextifierIntegration::class]) && null !== $this->options->getContextLines();
