...
final class Options
{
    ...
    {
        @\trigger_error(\sprintf('Method %s() is deprecated since version 2.3 and will be removed in 3.0. Use the "Sentry\\Integration\\IgnoreErrorsIntegration" integration instead.', __METHOD__), \E_USER_DEPRECATED);
     ...
     * @deprecated since version 2.3, to be removed in 3.0
     */
    public function isExcludedException(\Throwable $exception, bool $throwDeprecation = \true) : bool
    {
        if ($throwDeprecation) {
            @\trigger_error(\sprintf('Method %s() is deprecated since version 2.3 and will be removed in 3.0. Use the "Sentry\\Integration\\IgnoreErrorsIntegration" integration instead.', __METHOD__), \E_USER_DEPRECATED);
        }
    ...
    {
        @\trigger_error(\sprintf('Method %s() is deprecated since version 2.4 and will be removed in 3.0. Use the getDsn() method instead.', __METHOD__), \E_USER_DEPRECATED);
     ...
     */
    public function getDsn(bool $returnAsString = \true)
    {
        /** @var Dsn|null $dsn */
        ...
        if ($returnAsString) {
            @\trigger_error(\sprintf('Calling the method %s() and expecting it to return a string is deprecated since version 2.4 and will stop working in 3.0.', __METHOD__), \E_USER_DEPRECATED);
    ...
    {
        $resolver->setDefaults(['integrations' => [], 'default_integrations' => \true, 'send_attempts' => 3, 'prefixes' => \explode(\PATH_SEPARATOR, \get_include_path()), 'sample_rate' => 1, 'attach_stacktrace' => \false, 'context_lines' => 5, 'enable_compression' => \true, 'environment' => $_SERVER['SENTRY_ENVIRONMENT'] ?? null, 'project_root' => null, 'logger' => 'php', 'release' => $_SERVER['SENTRY_RELEASE'] ?? null, 'dsn' => $_SERVER['SENTRY_DSN'] ?? null, 'server_name' => \gethostname(), 'before_send' => static function (\Sentry\Event $event) : Event {
            return $event;
        }, 'tags' => [], 'error_types' => \E_ALL, 'max_breadcrumbs' => self::DEFAULT_MAX_BREADCRUMBS, 'before_breadcrumb' => static function (\Sentry\Breadcrumb $breadcrumb) : Breadcrumb {
            return $breadcrumb;
        }, 'excluded_exceptions' => [], 'in_app_exclude' => [], 'in_app_include' => [], 'send_default_pii' => \false, 'max_value_length' => 1024, 'http_proxy' => null, 'capture_silenced_errors' => \false, 'max_request_body_size' => 'medium', 'class_serializers' => []]);
        ...
        $resolver->setNormalizer('project_root', function (\WPSentry\ScopedVendor\Symfony\Component\OptionsResolver\Options $options, ?string $value) {
            if (null === $value) {
                return null;
            }
            @\trigger_error('The option "project_root" is deprecated. Use the "in_app_include" option instead.', \E_USER_DEPRECATED);
    ...
    private function validateIntegrationsOption($integrations) : bool
    {
        ...
        foreach ($integrations as $integration) {
            if (!$integration instanceof \Sentry\Integration\IntegrationInterface) {
                return \false;
            }
        }
        return \true;
    ...
    private function getDefaultIntegrations() : array
    {
        ...
        if (null === $this->defaultIntegrations) {
            $this->defaultIntegrations = [new \Sentry\Integration\ExceptionListenerIntegration(), new \Sentry\Integration\ErrorListenerIntegration(null, \false), new \Sentry\Integration\FatalErrorListenerIntegration(), new \Sentry\Integration\RequestIntegration(), new \Sentry\Integration\TransactionIntegration(), new \Sentry\Integration\FrameContextifierIntegration()];
        }
