...
final class Event implements \JsonSerializable
{
     ...
     */
    public function __construct(?\Sentry\EventId $eventId = null)
    {
        $this->id = $eventId ?? \Sentry\EventId::generate();
        $this->timestamp = \gmdate('Y-m-d\\TH:i:s\\Z');
        $this->level = \Sentry\Severity::error();
        $this->serverOsContext = new \Sentry\Context\ServerOsContext();
        $this->runtimeContext = new \Sentry\Context\RuntimeContext();
        $this->userContext = new \Sentry\Context\UserContext();
        $this->extraContext = new \Sentry\Context\Context();
        $this->tagsContext = new \Sentry\Context\TagsContext();
     ...
     */
    public function getId(bool $returnAsString = \true)
    {
        if ($returnAsString) {
     ...
     */
    public function getExtraContext() : \Sentry\Context\Context
    {
     ...
     * Gets a list of tags.
     */
    public function getTagsContext() : \Sentry\Context\TagsContext
    {
     ...
     */
    public function getUserContext() : \Sentry\Context\UserContext
    {
     ...
     */
    public function getServerOsContext() : \Sentry\Context\ServerOsContext
    {
    ...
    {
        $data = ['event_id' => (string) $this->id, 'timestamp' => $this->timestamp, 'level' => (string) $this->level, 'platform' => 'php', 'sdk' => ['name' => $this->sdkIdentifier, 'version' => $this->getSdkVersion()]];
        if (null !== $this->logger) {
        ...
        foreach (\array_reverse($this->exceptions) as $exception) {
            $exceptionData = ['type' => $exception['type'], 'value' => $exception['value']];
            if (isset($exception['stacktrace'])) {
                $exceptionData['stacktrace'] = ['frames' => $exception['stacktrace']->toArray()];
            }
            $data['exception']['values'][] = $exceptionData;
        }
        if (null !== $this->stacktrace) {
            $data['stacktrace'] = ['frames' => $this->stacktrace->toArray()];
        }
        ...
        if (null !== $this->message) {
            if (empty($this->messageParams)) {
                ...
                $data['message'] = ['message' => $this->message, 'params' => $this->messageParams, 'formatted' => $this->messageFormatted ?? \vsprintf($this->message, $this->messageParams)];
            }
        }
        return $data;
    }
