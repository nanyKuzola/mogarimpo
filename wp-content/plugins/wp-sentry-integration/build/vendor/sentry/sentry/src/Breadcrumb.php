...
final class Breadcrumb implements \JsonSerializable
{
     ...
     */
    private const ALLOWED_LEVELS = [self::LEVEL_DEBUG, self::LEVEL_INFO, self::LEVEL_WARNING, self::LEVEL_ERROR, self::LEVEL_CRITICAL, self::LEVEL_FATAL];
    /**
    ...
    public function __construct(string $level, string $type, string $category, ?string $message = null, array $metadata = [])
    {
        if (!\in_array($level, self::ALLOWED_LEVELS, \true)) {
            throw new \Sentry\Exception\InvalidArgumentException('The value of the $level argument must be one of the Breadcrumb::LEVEL_* constants.');
        }
        $this->type = $type;
        $this->level = $level;
    ...
    {
        return ['type' => $this->type, 'category' => $this->category, 'level' => $this->level, 'message' => $this->message, 'timestamp' => $this->timestamp, 'data' => $this->metadata];
    }
    /**
    ...
    {
        return new self($data['level'], $data['type'] ?? self::TYPE_DEFAULT, $data['category'], $data['message'] ?? null, $data['data'] ?? []);
    }
    /**
     * {@inheritdoc}
     *
