 ...
 * @since   1.0
 */
class Client implements \WPSentry\ScopedVendor\Http\Client\HttpClient, \WPSentry\ScopedVendor\Http\Client\HttpAsyncClient
{
    /**
     * cURL options.
     *
     ...
     */
    public function __construct(\WPSentry\ScopedVendor\Psr\Http\Message\ResponseFactoryInterface $responseFactory = null, \WPSentry\ScopedVendor\Psr\Http\Message\StreamFactoryInterface $streamFactory = null, array $options = [])
    {
        $this->responseFactory = $responseFactory ?: \WPSentry\ScopedVendor\Http\Discovery\Psr17FactoryDiscovery::findResponseFactory();
        $this->streamFactory = $streamFactory ?: \WPSentry\ScopedVendor\Http\Discovery\Psr17FactoryDiscovery::findStreamFactory();
    ...
    public function sendRequest(\WPSentry\ScopedVendor\Psr\Http\Message\RequestInterface $request) : \WPSentry\ScopedVendor\Psr\Http\Message\ResponseInterface
    {
        ...
        switch ($errno) {
            ...
            case \CURLE_SSL_CONNECT_ERROR:
                throw new \WPSentry\ScopedVendor\Http\Client\Exception\NetworkException(\curl_error($this->handle), $request);
            default:
                throw new \WPSentry\ScopedVendor\Http\Client\Exception\RequestException(\curl_error($this->handle), $request);
        }
     ...
     */
    private function createResponseBuilder() : \WPSentry\ScopedVendor\Http\Client\Curl\ResponseBuilder
    {
        $body = $this->streamFactory->createStreamFromFile('php://temp', 'w+b');
        $response = $this->responseFactory->createResponse(200)->withBody($body);
        return new \WPSentry\ScopedVendor\Http\Client\Curl\ResponseBuilder($response);
    }
    /**
     ...
     * @throws Exception\RequestException On invalid request.
     */
    private function prepareRequestOptions(\WPSentry\ScopedVendor\Psr\Http\Message\RequestInterface $request, \WPSentry\ScopedVendor\Http\Client\Curl\ResponseBuilder $responseBuilder) : array
    {
        $curlOptions = $this->curlOptions;
        try {
            $curlOptions[\CURLOPT_HTTP_VERSION] = $this->getProtocolVersion($request->getProtocolVersion());
        } catch (\UnexpectedValueException $e) {
            throw new \WPSentry\ScopedVendor\Http\Client\Exception\RequestException($e->getMessage(), $request);
        }
     ...
     * @return array cURL options based on request.
     */
    private function addRequestBodyOptions(\WPSentry\ScopedVendor\Psr\Http\Message\RequestInterface $request, array $curlOptions) : array
    {
        /*
     ...
     * @return string[]
     */
    private function createHeaders(\WPSentry\ScopedVendor\Psr\Http\Message\RequestInterface $request, array $curlOptions) : array
    {
        $curlHeaders = [];
    ...
    {
        if (!$this->multiRunner instanceof \WPSentry\ScopedVendor\Http\Client\Curl\MultiRunner) {
            $this->multiRunner = new \WPSentry\ScopedVendor\Http\Client\Curl\MultiRunner();
        }
        $handle = \curl_init();
        $responseBuilder = $this->createResponseBuilder();
        $requestOptions = $this->prepareRequestOptions($request, $responseBuilder);
        \curl_setopt_array($handle, $requestOptions);
        $core = new \WPSentry\ScopedVendor\Http\Client\Curl\PromiseCore($request, $handle, $responseBuilder);
        $promise = new \WPSentry\ScopedVendor\Http\Client\Curl\CurlPromise($core, $this->multiRunner);
