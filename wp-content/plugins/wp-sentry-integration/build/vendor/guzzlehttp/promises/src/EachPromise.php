 ...
 */
class EachPromise implements \WPSentry\ScopedVendor\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator */
    private $iterable;
    ...
    {
        $this->mutex = \false;
        $this->aggregate = new \WPSentry\ScopedVendor\GuzzleHttp\Promise\Promise(function () {
            \reset($this->pending);
            ...
            while ($promise = \current($this->pending)) {
                \next($this->pending);
                $promise->wait();
                if ($this->aggregate->getState() !== \WPSentry\ScopedVendor\GuzzleHttp\Promise\PromiseInterface::PENDING) {
                    return;
                }
            }
        });
